...
experiment <- function(nsim, n, phi_mean, phi_sd, y_sample_size) {
  ...
  # Without aggregation
  y <- replicate(nsim, rbinom(y_sample_size, m, rho), simplify = FALSE)
  y <- lapply(y, sum)
  
  # With aggregation (at the level of the latent field)
  y_agg <- replicate(nsim, rbinom(1, sum(m), mean(rho)), simplify = FALSE)
...
* Aggregation over-estimates the truth
* Unsure about the difference between smaller $\rho$ and $\rho \approx 0.5$ settings. A lot of the apparent difference is because you get more samples as $\rho$ is larger the the differences become clearer. Could increase play with settings more on this
